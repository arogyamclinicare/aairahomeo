...
function TreatmentCard({ treatment, index }: { treatment: typeof treatments[0]; index: number }) {
  ...
  return (
    ...
    >
      <Card className="p-6 hover:shadow-2xl transition-all duration-500 border-emerald-100 hover:border-emerald-300 bg-white/80 backdrop-blur-sm group cursor-pointer hover:-translate-y-2 relative overflow-hidden">
        {/* Shine effect on hover */}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
        
        <div className="relative flex items-start gap-4">
          <div className={`w-14 h-14 rounded-xl ${colors.bg} flex items-center justify-center flex-shrink-0 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-lg`}>
            <Icon className={`w-7 h-7 ${colors.icon}`} />
          </div>
          <div className="flex-1">
            <h3 className="text-gray-900 mb-2 group-hover:text-emerald-700 transition-colors">{treatment.title}</h3>
    ...
    <section id="treatments" className="py-20 sm:py-24 md:py-32 bg-gradient-to-br from-gray-50 via-white to-emerald-50 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-gray-900 mb-4">Treatments & Specialties</h2>
